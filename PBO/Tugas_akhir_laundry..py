...
class Transaksi():
    ...
    def Laundry(self):
            ...
            # self.Entry_search=tk.Entry(self.labelframe2,width=70,bd=3)
            # self.Entry_search.grid(column=0,row=0,padx=100,pady=60)

            # self.Search_button=tk.Button(self.labelframe2,text='Search',relief='groove',bg='SteelBlue2',fg='white',font='verdana  10 bold ',padx=45,pady=2)
            # self.Search_button.place(x=240,y=100)
            ...
            #Membuat koneksi dan membuat database bernama Data
            file=sqlite3.connect('Data.db')
            #Menampung data hasil seleksi
            self.c=file.cursor()

            #execute,untuk menjalankan perintah sql
            #create table,Membuat table dengan kolom nama dll. If not exists artinya jika table sudah ada maka tidak perlu dibuat lagi
            self.c.execute('''CREATE TABLE IF NOT EXISTS new(
                    Nama text,
                    Alamat text,
                    No_Hp integer,
                    Jenis text,
                    ...
                    Id_Laundry integer,
                    Berat_Pakaian integer,
                    Layanan_Antar_Jemput text,
                    Diskon integer,
                    Total_Harga integer,
                    Status text
                    )''')
            #commit,untuk menerapkan perubahan ke database
            file.commit()
            #Menutup file
            file.close()
            ...
            #Menampung data hasil seleksi
            self.c=file.cursor()
            #select * from,perintah sql untuk menampilkan data dari tabel database
            #execute,untuk menjalankan perintah sql
            self.c.execute('SELECT * FROM new')
            #fetchall,untuk menampilakn semua data yg sudah diinssertkan yang ada dalam database
            rows = self.c.fetchall()
            #line 290 untuk memasukkan data yg ada didalam database ke tabel treeview
            for row in rows:
                self.tree2.insert('',tk.END, text=row[0],values=(row[1],row[2],row[3],row[4],row[5],row[6],row[7],row[8],row[9],row[10],row[11]))
            self.c.close()
            #close,untuk menutup cursor
            ...
            def insert_data():
                ...
                #menampung data hasil seleksi
                self.c=file.cursor()
                #execute, untuk mengeksekusi perintah sql
                #Insert into, untuk memasukkan input entry ke database
                self.c.execute('''INSERT INTO new VALUEs(:Nama,:Alamat,:No_Hp,:Jenis,:Harga,:Tanggal_Transaksi,:Id_Laundry,:Berat_Pakaian,:Layanan_Antar_Jemput,:Diskon,:Total_Harga,:Status)''',
                {
                    'Nama': self.Entry_nama.get(),
                    'Alamat': self.Entry_alamat.get(),
                    'No_Hp': self.Entry_no_hp.get(),
                    'Jenis': self.Box_jenis.get(),
                    'Harga': Harga.get(),
                    'Tanggal_Transaksi': self.Entry_tgl_transaksi.get(),
                    'Id_Laundry': self.Entry_id_laundry.get(),
                    'Berat_Pakaian': self.Entry_berat_pakaian.get(),
                    'Layanan_Antar_Jemput': self.Box_antar_jemput.get(),
                    'Diskon': Diskon.get(),
                    'Total_Harga': Total.get(),
                    'Status': self.Box_status.get()
                })
                #commit,untuk menerapkan perubahan kedatabase
                file.commit()
                #close,untuk menutup cursor
                file.close()
                ...
                showinfo(title='Insert',message='Data Berhasil Di Tambahkan')
                ...
                # except:
                #     showerror(title='Erorr',message='Masukkan Data Sesuai Entry')
            ...
            # self.clear_button.grid(row=0,column=1,padx=0,pady=0)
            # self.clear_button.config(height=2,width=16)
            ...
            def delete():
                ...
                file.commit()
                file.close()
                showinfo(title='Delet',message='Data Berhasil Di Hapus')
                ...
                # a=self.tree2.selection()[0]  
                # for a in self.tree2.selection():
                #     self.c.execute("DELETE FROM new WHERE id=?",
                #                         (self.tree2.set(a, "expense type"),))
                # file.commit()
                # self.tree2.delete(a)
 

                # self.c.execute('DELETE from new WHERE Id_Laundry=', + self.Entry_id_laundry.get() )

                # file.commit()
                # file.close()

            self.remove_button=tk.Button(self.labelframe4,text='DELETE',relief='groove',bg='SteelBlue2',fg='white',font='verdana  10 bold ',command=delete,padx=40,pady=2)
            ...
            self.refresh_button=tk.Button(self.labelframe4,text='REFRESH',relief='groove',bg='SteelBlue2',fg='white',font='verdana  10 bold ',command=refresh,padx=40,pady=2)
            self.refresh_button.grid(row=0,column=2,padx=0,pady=0)
            self.refresh_button.config(height=2,width=23)
